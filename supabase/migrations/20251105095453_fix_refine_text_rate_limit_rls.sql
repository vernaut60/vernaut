-- Migration: Fix and enhance refine-text rate limiting with RLS and cleanup
-- This migration adds:
--  - RLS policies for rate_limits and refine_cache tables
--  - Fixed check_rate_limit() function with SECURITY DEFINER
...
DROP POLICY IF EXISTS "Users can view their own cache" ON public.refine_cache;
DROP POLICY IF EXISTS "Users can insert their own cache" ON public.refine_cache;
DROP POLICY IF EXISTS "Users can delete their own cache" ON public.refine_cache;
...
-- 4) Replace function with SECURITY DEFINER version (fixes previous syntax error)
...
-- 5) Cleanup function for old rate limit windows (older than 24 hours)
...
-- 6) Cleanup function for old cache entries (older than 7 days)
