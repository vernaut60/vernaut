...
-- Step 4: Add composite indexes for dashboard queries (only if ideas table exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
...
-- Step 7: Add data integrity constraints (only if ideas table exists)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
...
-- Score range constraints (only if columns exist)
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.tables 
    ...
    -- Check if score column exists before adding constraint
    IF EXISTS (
    ...
    -- Check if risk_score column exists before adding constraint
    IF EXISTS (
      SELECT 1 FROM information_schema.columns 
      WHERE table_schema = 'public' 
      AND table_name = 'ideas' 
